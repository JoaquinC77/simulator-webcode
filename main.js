...
const handleElementDisplay = (element, typeElement) => {
  ...
  const buttonHeaderHtml = select(`#${TYPES_IDS.html}`)
  const buttonHeaderCSS = select(`#${TYPES_IDS.css}`)
  const buttonHeaderJs = select(`#${TYPES_IDS.js}`)

  if (typeElement === TYPES_IDS.html) {
    cssEl.classList.replace('box-code-flex', 'box-code')
    jsEl.classList.replace('box-code-flex', 'box-code')
    buttonHeaderJs.classList.remove('item-select')
    buttonHeaderCSS.classList.remove('item-select')

    if (htmlEl.classList[0] === 'box-code') {
      htmlEl.classList.replace('box-code', 'box-code-flex')
      buttonHeaderHtml.classList.add('item-select')
      return
    }
  ...
  if (typeElement === TYPES_IDS.css) {
    ...
    jsEl.classList.replace('box-code-flex', 'box-code')
    buttonHeaderJs.classList.remove('item-select')
    buttonHeaderHtml.classList.remove('item-select')

    if (cssEl.classList[0] === 'box-code') {
      cssEl.classList.replace('box-code', 'box-code-flex')
      buttonHeaderCSS.classList.add('item-select')
      return
    }
  ...
  if (typeElement === TYPES_IDS.js) {
    ...
    if (jsEl.classList[0] === 'box-code') {
      jsEl.classList.replace('box-code', 'box-code-flex')
      buttonHeaderJs.classList.add('item-select')
      return
    }
...
function handleClickHeaderItems (headerItems = []) {
  headerItems.forEach(element => {
    element.addEventListener('click', (e) => {
      handleElementDisplay(e.target, element.id)
    })
  })
}
